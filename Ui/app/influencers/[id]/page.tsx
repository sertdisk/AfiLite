...
export default function InfluencerDetailPage() {
  ...
  async function onSave(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await patchAdminInfluencerDetail(id, body);

      setSuccessMsg('Profil güncellendi.');
      // Güncel veriyi tekrar çek
      await fetchDetail();
    } catch (e: any) {
      setError(e.message || 'Beklenmeyen bir hata oluştu (güncelleme).');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function fetchAll(ctrl?: AbortController) {
    ...
    try {
      const [summary, history] = await Promise.all([
        getAdminInfluencerBalance(Number(influencerId)),
        getInfluencerBalanceHistory(Number(influencerId)),
      ]);
