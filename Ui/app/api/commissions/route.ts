/**
 * Proxy: /api/commissions → BACKEND /api/v1/commissions
...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `${BACKEND_ORIGIN}/api/v1/commissions${qs ? `?${qs}` : ''}`;
    ...
    const message = err?.message || 'Proxy error (GET /api/commissions)';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = `${BACKEND_ORIGIN}/api/v1/commissions`;
    const res = await fetch(url, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ message: msg || 'Komisyon oluşturma başarısız.' }), { status: res.status, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
    }
    return new Response(text, { status: res.status, headers: { 'Content-Type': contentType } });
  } catch (err: any) {
    const message = err?.message || 'Proxy error (POST /api/commissions)';
    return new Response(JSON.stringify({ message }), { status: 500, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
  }
}