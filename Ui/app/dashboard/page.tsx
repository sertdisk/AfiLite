...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const arr = await getAdminPendingCodes();
        if (!ignore) {
          setPendingCodes(arr);
          setCodesError(null);
        }
  ...
  async function resolveCodeInfo(code: string) {
    ...
    try {
      const { code: detail } = await searchAdminCode(code);
    ...
    (async () => {
      try {
        ...
        try {
          ...
          const total = stats?.stats?.total_commission;
  ...
  async function submitSale(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await postAdminSale(body);
      setSaleMsg('Satış kaydedildi.');
      setQProduct('');
    } catch (err: any) {
      setSaleErr(err.message || 'Beklenmeyen bir hata oluştu.');
    } finally {
      setSaleBusy(false);
    }
  }
